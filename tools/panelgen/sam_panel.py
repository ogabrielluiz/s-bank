...
class Panel:
    ...
    def svg(self) -> str:
        ...
        mx = 5 if w < 50 else 7          # side margin
        ...
        if narrow:
            ...
            o.append(text_paths(f"{self.hp}HP / S-{self.serial}", w - mx, 5.4, 1.5, gray, "end", weight=0.18))
            if self.sub:
                halves = [s.strip() for s in self.sub.split("|")]
                o.append(text_paths(halves[0], w - mx, 8.0, 1.4, gray, "end", weight=0.16))
                if len(halves) > 1:
                    o.append(text_paths(halves[1], w - mx, 10.4, 1.4, gray, "end", weight=0.16))
